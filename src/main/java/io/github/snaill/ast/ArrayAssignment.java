...
/**
 * Представляет присваивание элементу массива в AST.
 * Генерирует байткод для установки значения элемента массива по индексу.
 */
public class ArrayAssignment extends AbstractNode implements Statement {
    public ArrayAssignment(Expression array, Expression index, Expression value) {
        super(List.of(array, index, value));
    }
    ...
    public void check(Scope scope) throws io.github.snaill.exception.FailedCheckException {
        ...
        Type valueType = getValue().getType(scope);
        Type elementType = at.getElementType();
        if (!valueType.equals(elementType)) {
            String before = getValue().getSource() != null ?
                io.github.snaill.ast.SourceBuilder.toSourceLine(getValue().getSource(), getValue().getLine(), getValue().getCharPosition(), getValue().toString().length()) :
                io.github.snaill.ast.SourceBuilder.toSourceCode(getValue());
            throw new io.github.snaill.exception.FailedCheckException(
                new io.github.snaill.result.CompilationError(
                    io.github.snaill.result.ErrorType.TYPE_MISMATCH,
                    before,
                    "Array element type mismatch: expected " + elementType + ", got " + valueType,
                    ""
                ).toString()
            );
        }
